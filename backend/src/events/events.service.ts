...
export class EventsService {
    ...
    setImmediate(() => {
      (async () => {
        ...
        if (updatedEvent.videoRoom?.externalId &&
            updatedEvent.videoRoom.provider === VideoProvider.MY_OWN_CONFERENCE) {
          try {
            ...
            this.logger.error('Failed to update MyOwnConference webinar room', error);
          }
        }
      })();
    });
  ...
  private async deleteVideoRoom(videoRoomId: string): Promise<void> {
    try {
      ...
      if (videoRoom && videoRoom.externalId) {
        if (videoRoom.provider === VideoProvider.MY_OWN_CONFERENCE) {
  ...
  private buildJitsiRoom(eventId: string): { url: string; roomName: string } {
    ...
    return { roomName, url: `${base}/${roomName}` };
  }
